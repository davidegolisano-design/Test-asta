...
(function () {
  ...
  async function read(id) {
    ...
    try {
      const {data,error} = await client.from(TABLE).select('room_id,environment,features,revision,updated_at')
  ...
  function offer(id) {
    ...
    $('premium-activation').onclick = async () => {
      ...
      else { $('premium-activation-info').hidden = false; $('premium-activation-info').scrollIntoView({block:'nearest'}); }
    };
    show(el);
  }
  ...
  function renderAdmin(error = '') {
    ...
    el.innerHTML = `<header class="premium-dialog-head"><span class="premium-label">✦ PREMIUM · SUPERUSER</span>${closeButton()}</header>
      ...
      <p class="premium-free-note">Classic, Mantra e miniature generiche: sempre gratuiti.</p></div>
  ...
  async function openAdmin(id) {
    ...
    const ticket = ++adminGeneration; adminRoom = room; adminRow = null; adminBusy = false;
    renderAdmin(); show(dialog('premium-admin-dialog'));
    try { const row = await read(id); if (ticket !== adminGeneration) return; adminRow = row; renderAdmin(); }
