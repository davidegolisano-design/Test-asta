...
// === liveasta-v73-theme-runtime ===
(function(){
  const THEME_KEY='liveasta_theme';
  const THEMES=['broadcast','stadium','carbon','light-neutral','light-blue','light-sand','ocean-daily','ocean-dark','sunset-daily','sunset-dark','royal-daily','royal-dark'];
  const META_COLORS={
    broadcast:'#0D1117',
    stadium:'#071521',
    carbon:'#0E0E0F',
  ...
  function normalizeTheme(theme){
    if(theme==='daily')theme='broadcast';
    return THEMES.includes(theme)?theme:'broadcast';
  }
  ...
  function updateThemeMeta(theme){
    const color=META_COLORS[theme]||META_COLORS.broadcast;
    ...
    if(!ms){
      ...
      ms.name='msapplication-navbutton-color';
  ...
  window.applyLiveAstaTheme=function(theme,persist=true){
    theme=normalizeTheme(theme);
    document.documentElement.setAttribute('data-live-theme',theme);
    ...
    if(currentLabel){const labels={broadcast:'Classic Dark',stadium:'Glow Electric Dark',carbon:'Modern Dark','light-neutral':'Classic Daily','light-blue':'Glow Electric Daily','light-sand':'Modern Daily','ocean-daily':'Ocean Daily','ocean-dark':'Ocean Dark','sunset-daily':'Sunset Daily','sunset-dark':'Sunset Dark','royal-daily':'Royal Daily','royal-dark':'Royal Dark'};currentLabel.textContent=labels[theme]||'Tema';}
    return theme;
  };

  const THEME_LABELS={broadcast:'Classic Dark',stadium:'Glow Electric Dark',carbon:'Modern Dark','light-neutral':'Classic Daily','light-blue':'Glow Electric Daily','light-sand':'Modern Daily','ocean-daily':'Ocean Daily','ocean-dark':'Ocean Dark','sunset-daily':'Sunset Daily','sunset-dark':'Sunset Dark','royal-daily':'Royal Daily','royal-dark':'Royal Dark'};

  function updateCurrentThemeLabel(theme){
    const el=document.getElementById('current-theme-label');
    if(el)el.textContent=THEME_LABELS[normalizeTheme(theme)]||'Tema';
  }
  ...
  window.setLiveAstaTheme=function(theme){
    const applied=window.applyLiveAstaTheme(theme,true);
    updateCurrentThemeLabel(applied);
  };

  function bootTheme(){
    let theme='broadcast';
    try{theme=normalizeTheme(localStorage.getItem(THEME_KEY)||'broadcast')}catch(_){}
    window.applyLiveAstaTheme(theme,false);
  }

  function reapplySystemTheme(){
    let theme='broadcast';
    try{theme=normalizeTheme(localStorage.getItem(THEME_KEY)||'broadcast')}catch(_){}
    updateThemeMeta(theme);
  }
  ...
  if(document.readyState==='loading'){
    document.addEventListener('DOMContentLoaded',()=>{
      bootTheme();
      reapplySystemTheme();
      setTimeout(reapplySystemTheme,120);
      setTimeout(reapplySystemTheme,650);
    },{once:true});
  }else{
    bootTheme();
    reapplySystemTheme();
  ...
  window.addEventListener('pageshow',reapplySystemTheme);
  document.addEventListener('visibilitychange',()=>{
    if(!document.hidden)reapplySystemTheme();
  });
})();
