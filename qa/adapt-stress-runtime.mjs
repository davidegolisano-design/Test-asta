...
s=s.replace(
  "for(let i=0;i<3&&!await page.locator('#screen-player-buzzer.active').count();i++){await page.locator('#player-wizard-next:visible,#player-enter-btn:visible').first().click();await sleep(750);}",
  "for(let i=0;i<3&&!await page.locator('#screen-player-buzzer.active').count();i++){await page.evaluate(()=>window.playerAccessWizardNext?.());await sleep(1000);}");
