...
(async()=>{
  ...
  ({rows}=await call('all',true));assert.equal(rows[0].result.features.length,7);
  ...
  assert.equal(rows.length,2);assert.equal(rows[0].features.length,7);assert.deepEqual(rows[1].features,['chat']);
