...
(async()=>{
  ...
  await db.exec(`alter table public.fanta_rooms add column timer_seconds integer default 5,
    add column prep_seconds integer default 5, add column sealed_timer_seconds integer default 30,
    add column sealed_reveal_seconds integer default 5;`);
  await db.exec(fs.readFileSync('supabase/premium-roster-io.sql','utf8'));
  ...
  assert.deepEqual([r.prep_seconds,r.timer_seconds,r.sealed_timer_seconds,r.sealed_reveal_seconds],[3,8,40,3]);
  ...
  const full=(await feature('all',true,r.id)).rows[0].r;
  assert.equal(full.features.length,8);assert.ok(full.features.includes('roster_io'));
  await feature('roster_io',false,r.id);await feature('chat',false,r.id);
