...
create table public.liveasta_premium_entitlements (
  ...
  features text[] not null default '{}'
    check (features <@ array['sealed','random','turns','ready','budget','chat']::text[]),
...
begin
  ...
  if p_environment is null or p_environment not in ('dev-premium','production')
     or p_feature is null or p_feature not in ('all','sealed','random','turns','ready','budget','chat')
  ...
  if p_feature='all' then
    v_features := case when p_enabled then array['sealed','random','turns','ready','budget','chat']::text[] else '{}'::text[] end;
