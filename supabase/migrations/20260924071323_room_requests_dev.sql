-- Prerequisite: premium-dev.sql. Pending release migration, NOT applied.
-- New endpoints support separate dev/production entitlements and notifications.
-- Existing room approval defaults and legacy endpoints remain unchanged.
...
create table liveasta_premium_private.creation_keys (
  ...
  primary key(request_id,environment)
);
...
begin
  ...
  perform net.http_post(
    url:='https://qvkembahfeecfpsshepv.supabase.co/functions/v1/liveasta-notification',
...
create function liveasta_premium_private.create_room(
  p_name text,p_room_password text,p_email text,p_config jsonb,p_request_id uuid,p_environment text
...
begin
  if p_environment is null or p_environment not in ('dev-premium','production')
     or p_request_id is null or length(v_name) not between 1 and 30
  ...
  perform pg_advisory_xact_lock(hashtextextended(p_environment||p_request_id::text,0));
  select r.* into v_room from liveasta_premium_private.creation_keys k
    join public.fanta_rooms r on r.id=k.room_id where k.request_id=p_request_id and k.environment=p_environment;
  if found then
  ...
  insert into liveasta_premium_private.creation_keys(request_id,environment,room_id) values(p_request_id,p_environment,v_room.id);
  insert into liveasta_premium_private.notifications(room_id,environment,kind,payload)
    values(v_room.id,p_environment,'room_created',jsonb_build_object(
...
revoke all on function liveasta_premium_private.create_room(text,text,text,jsonb,uuid,text) from public;
grant execute on function liveasta_premium_private.create_room(text,text,text,jsonb,uuid,text) to anon,authenticated;
create function public.liveasta_create_room(p_name text,p_room_password text,p_email text,p_config jsonb,p_request_id uuid,p_environment text)
returns jsonb language sql security invoker set search_path='' as $$
  select liveasta_premium_private.create_room(p_name,p_room_password,p_email,p_config,p_request_id,p_environment);
$$;
revoke all on function public.liveasta_create_room(text,text,text,jsonb,uuid,text) from public;
grant execute on function public.liveasta_create_room(text,text,text,jsonb,uuid,text) to anon,authenticated;
...
create function liveasta_premium_private.request_premium(p_room_id uuid,p_environment text,p_room_password text,p_email text)
returns jsonb language plpgsql security definer set search_path='' as $$
...
revoke all on function liveasta_premium_private.request_premium(uuid,text,text,text) from public;
grant execute on function liveasta_premium_private.request_premium(uuid,text,text,text) to anon,authenticated;
create function public.liveasta_request_premium(p_room_id uuid,p_environment text,p_room_password text,p_email text default null)
returns jsonb language sql security invoker set search_path='' as $$
  select liveasta_premium_private.request_premium(p_room_id,p_environment,p_room_password,p_email);
$$;
revoke all on function public.liveasta_request_premium(uuid,text,text,text) from public;
grant execute on function public.liveasta_request_premium(uuid,text,text,text) to anon,authenticated;
...
begin
  ...
  return jsonb_build_object('id',v_job.id,'room_id',v_job.room_id,'kind',v_job.kind,
    'environment',v_job.environment,'cycle',v_job.cycle,'payload',v_job.payload);
end $$;
create function public.liveasta_claim_notification(p_id uuid,p_token uuid,p_claim uuid)
returns jsonb language sql security invoker set search_path='' as $$
...
create function public.liveasta_finish_notification(p_id uuid,p_claim uuid,p_state text,p_error text default null)
returns boolean language sql security invoker set search_path='' as $$
  select liveasta_premium_private.finish_notification(p_id,p_claim,p_state,p_error);
$$;
revoke all on function liveasta_premium_private.claim_notification(uuid,uuid,uuid),public.liveasta_claim_notification(uuid,uuid,uuid),
 liveasta_premium_private.finish_notification(uuid,uuid,text,text),public.liveasta_finish_notification(uuid,uuid,text,text)
 from public,anon,authenticated;
grant usage on schema liveasta_premium_private to service_role;
grant execute on function liveasta_premium_private.claim_notification(uuid,uuid,uuid),public.liveasta_claim_notification(uuid,uuid,uuid),
 liveasta_premium_private.finish_notification(uuid,uuid,text,text),public.liveasta_finish_notification(uuid,uuid,text,text) to service_role;

create function liveasta_premium_private.admin_notifications(p_room_id uuid,p_environment text,p_password text,p_retry boolean)
returns jsonb language plpgsql security definer set search_path='' as $$
...
begin
  ...
  update liveasta_premium_private.notifications set state='uncertain',error_code='WORKER_INTERRUPTED'
    where room_id=p_room_id and environment=p_environment and state='sending' and claimed_at<now()-interval '2 minutes';
  if p_retry then
    for v_job in select id from liveasta_premium_private.notifications
      where room_id=p_room_id and environment=p_environment and state in ('pending','failed') loop
      perform liveasta_premium_private.queue_notification(v_job);
    end loop;
  end if;
  select coalesce(jsonb_agg(jsonb_build_object('kind',n.kind,'state',n.state,'created_at',n.created_at,'sent_at',n.sent_at)
    ...
    from liveasta_premium_private.notifications n where n.room_id=p_room_id and n.environment=p_environment;
  return v_result;
end $$;
revoke all on function liveasta_premium_private.admin_notifications(uuid,text,text,boolean) from public;
grant execute on function liveasta_premium_private.admin_notifications(uuid,text,text,boolean) to anon,authenticated;
create function public.liveasta_admin_premium_notifications(p_room_id uuid,p_environment text,p_password text,p_retry boolean default false)
returns jsonb language sql security invoker set search_path='' as $$
  select liveasta_premium_private.admin_notifications(p_room_id,p_environment,p_password,p_retry);
$$;
revoke all on function public.liveasta_admin_premium_notifications(uuid,text,text,boolean) from public;
grant execute on function public.liveasta_admin_premium_notifications(uuid,text,text,boolean) to anon,authenticated;
